...
export default function VehicleDetailsForm({ index, item, formData, setFormData, handleDetailsChange, errors }) {
  ...
  /** useStaet for image preview */
  const [previews, setPreviews] = useState([]);
  const fileInputRef = useRef(null); // Reference for hidden file input
  /** New state for address checkbox */
  ...
  /** Remove single image */
  const removeImage = (i) => {
    const newFiles = formData.image_ids.filter((_, idx) => idx !== i);
    const newPreviews = previews.filter((_, idx) => idx !== i);

    setFormData({ ...formData, image_ids: newFiles });
  ...
  return (
    <form className={styles.vehicleForm} encType="multipart/form-data">
      ...
      <div>
        ...
        <textarea className={`${styles.formControl} ${styles.reFormF}`} name="vehicle_description" placeholder="Vehicle Description" value={item.details?.vehicle_description || ""} onChange={(e) => handleDetailsChange(index, "vehicle_description", e.target.value)} />
    ...
    {/* ---------Image Upload + Custom Button + Preview ----*/}
      ...
      <div className={`${styles.row} ${styles.roh_fields}`}>
      <div className={`${styles.mb3} ${styles.roh_inoputField} w-100`}>
      ...
      <select className={`${styles.formControl} ${styles.reFormF}`} name="rental_period" value={item.details?.rental_period || ""} onChange={(e) => handleDetailsChange(index, "rental_period", e.target.value)}>
